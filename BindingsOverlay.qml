...
Rectangle {
  ...
  function conflict(mode, shortcut, detail) {
    root.stopCapture()
    root.pendingMode = mode
    root.pendingShortcut = shortcut
    root.conflictDetail = detail
    root.forceActiveFocus()
  }
  ...
  Column {
    ...
    Text {
      width: parent.width
      text: "Click Change, then press a shortcut. Existing bindings are preserved unless you explicitly approve an override."
      textFormat: Text.PlainText
    ...
    Repeater {
      ...
      delegate: BorderSurface {
        ...
        width: parent.width
        implicitHeight: row.implicitHeight + Style.space(20)
    ...
    BorderSurface {
      ...
      color: Util.alpha(Color.error, 0.06)
      borderSpec: Border.flat(Color.error, Style.normalBorderWidth)
      radius: Style.cornerRadius
      Column {
        id: conflictColumn
        anchors.left: parent.left
        ...
        Text {
          width: parent.width
          text: root.conflictDetail + "\nOverride it while Feed the Flock uses " + root.pendingShortcut + "?"
          textFormat: Text.PlainText
          wrapMode: Text.WordWrap
          color: root.foreground
          font.family: root.fontFamily
        ...
        Row {
          ...
          Button {
            ...
            foreground: root.foreground
            fontFamily: root.fontFamily
            onClicked: { root.pendingShortcut = ""; root.conflictDetail = "" }
          }
          Button {
            text: "Override"
            bordered: true
            ...
            fontFamily: root.fontFamily
            onClicked: {
              root.setRequested(root.pendingMode, root.pendingShortcut, true)
    ...
    Text {
      width: parent.width
      text: "Feed the Flock writes one managed Hyprland file and one loader line. Clearing both shortcuts leaves the integration installed but inactive."
      textFormat: Text.PlainText
      wrapMode: Text.WordWrap
      color: root.dimForeground
      font.family: root.fontFamily
