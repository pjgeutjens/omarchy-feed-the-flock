...
- Read-only discovery of agents reached by a running `herdr --remote` client
...
Feed the Flock discovers agents from the default running Herdr session. It also detects up to four `herdr --remote <ssh-target>` clients owned by the current user and reads their agent lists through bounded, non-interactive SSH snapshots. Remote rows are labeled **REMOTE** and **READ ONLY** in the target dropdown: they expose remote agent/context/status information but cannot be selected for delivery. The SSH target must already work with batch authentication, as it does for an active Herdr remote client; remote status is cached for ten seconds. Choose the delivery target with the compact dropdown in the plugin (or press `T` to open it), and the feed submits notes through Herdr's `agent prompt` API according to the selected mode. The workspace displays the selected target and provides explicit note- and section-level Feed Now actions. Clipboard remains the default; delivery is accepted only when the selected Herdr agent is idle or done.
